...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if the required user data exists
      if (!data.data || !data.data._id) {
        throw new Error("User ID is missing in response");
      }
  
      // Store user data in localStorage
      localStorage.setItem("userData", JSON.stringify(data.data)); // Store the entire user object
      localStorage.setItem("userId", data.data._id); // Store only the userId separately
  
      // Optionally, handle admin check
